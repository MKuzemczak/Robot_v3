...
/*template <typename Scalar, int RowsAtCompileTime, int ColsAtCompileTime>
    Eigen::Matrix <Scalar, ColsAtCompileTime, RowsAtCompileTime> pseudoInverse(Eigen::Matrix< Scalar, RowsAtCompileTime, ColsAtCompileTime> &m)
    {
        ...
        pcPort << "Eigen::pseudoInverse(): Transpose:\n" << transpose << '\n'; //debug

        Eigen::MatrixXd mult(newRows, newRows);
        mult = m0 * transpose;

        pcPort << "Eigen::pseudoInverse(): mult:\n" << mult << '\n'; //debug
        pcPort << "Eigen::pseudoInverse(): mult.determinant() == " << (double)mult.determinant() << "\n\n";
        if(mult.cols() == 3)
            pcPort << "Eigen::pseudoInverse(): det(mult)" << mult(0, 0)*mult(1, 1)*mult(2, 2)+mult(0, 1)*mult(1, 2)*mult(2, 0)+mult(0, 2)*mult(1, 0)*mult(2, 1)-
                mult(0, 2) *mult(1, 1)* mult(2, 0)- mult(0, 0) *mult(1, 2)* mult(2, 1)- mult(0, 1)* mult(1, 0)* mult(2, 2) << "\n\n";

        Eigen::MatrixXd inv;
        inv = mult.inverse();

        pcPort << "Eigen::pseudoInverse(): inv:\n" << inv << '\n'; //debug

        Eigen::MatrixXd result;
        result = transpose * inv;

        pcPort << "Eigen::pseudoInverse(): result:\n" << result << '\n'; //debug
